...
import { environmentView, workspaceView } from "../../core/views.js";
...
function listEnvironments(workspace: Workspace, asJson: boolean): void {
  ...
  emit(asJson, workspaceView(workspace), () => {
...
async function showEnvironment(workspace: Workspace, name: string, asJson: boolean): Promise<void> {
  const view = environmentView(await workspace.resolveEnvironment(name));
  const variables = view.variables;

  emit(asJson, { ...view, warnings: workspace.warnings }, () => {
    const width = Math.max(0, ...variables.map((v) => v.name.length)) + 2;
    return [
      `name:           ${view.name}`,
      `classification: ${view.classification}`,
      `baseUrl:        ${view.baseUrl ?? "(none)"}`,
      `allowedHosts:   ${view.allowedHosts.join(", ") || "(none — every request is refused)"}`,
      `auth:           ${view.auth?.type ?? "(none)"}`,
      "",
      "variables:",
      ...(variables.length > 0
